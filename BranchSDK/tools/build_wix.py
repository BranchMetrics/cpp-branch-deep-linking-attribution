...
def make_component_elem(elem, identifier, directory):
    ...
    # appear.
    return SubElement(elem, "Component", {"Id": identifier, "Directory": directory, "Guid": str(UUID())})
...
def all_subdirs(path):
    ...
    all_dirs = [f for f in allpaths if os.path.isdir(f)]

    # List of all subdirectories to all levels below path
    all_nested = [all_subdirs(d) for d in alldirs]
...
def wix_component(elem, path):
    # TODO: Determine the Id to use for each folder automatically.
    ...
    component = make_component_elem(elem, , file_identifier(path))

    for f in os.listdir(path):
        # TODO: Generate the Source for the file in the form
        # $(var.projectDir)\..\..\..\build\...
        file_elem = make_file_elem(component, file_identifier(path), )
